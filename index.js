...
// // ✅ Preflight request handle করার জন্য জরুরি
// app.options("*", cors());
...
// // ✅ বাইরে declare করা — run() এর ভেতরে assign হবে
// let mealsCollection;
...
//     const usersCollection = db.collection("users");
//     const requestsCollection = db.collection("requests");
//     mealsCollection = db.collection("meals")         // ✅ const নেই
//     const reviewsCollection = db.collection("reviews")
//     const favouriteCollection = db.collection("favourites")
//     const ordersCollection = db.collection("orders")
//     const paymentsCollection = db.collection("payments")
...
//         const user = await usersCollection.findOne({ email });
//         if (!user) return res.status(404).send({ error: "User not found" });
//         if (user.status === "fraud") return res.status(403).send({ error: "Fraud user - action blocked" });
//         next();
//       } catch (error) {
...
//       const email = req.params.email;
//       const result = await usersCollection.updateOne({ email }, { $set: { status: "fraud" } });
//       if (result.modifiedCount > 0) return res.send({ success: true });
//       res.send({ success: false });
//     });
...
//       try {
//         const meal = req.body;
//         if (req.decoded_email !== meal.userEmail) return res.status(403).send({ error: "Access denied" });
//         meal.createdAt = new Date();
//         const result = await mealsCollection.insertOne(meal);
//         res.send({ success: true, result });
//       } catch (error) {
...
//       });
//       if (exists) {
//         return res.send({ success: false, message: "Already you have added review to this meal" });
//       }
//       if (!ObjectId.isValid(review.foodId)) {
//         return res.status(400).send({ success: false, message: "Invalid foodId format" });
//       }
//       const result = await reviewsCollection.insertOne(review);
//       const allReviews = await reviewsCollection.find({ foodId: review.foodId }).toArray();
//       const total = allReviews.reduce((sum, r) => sum + Number(r.rating || 0), 0);
...
//     app.get("/reviews", async (req, res) => {
//       try {
//         const result = await reviewsCollection.find().sort({ date: -1 }).limit(10).toArray();
//         res.send(result);
//       } catch (err) {
//         console.error(err);
//         res.status(500).send({ error: "Failed to fetch reviews" });
...
//         mealId: favourite.mealId,
//       });
//       if (exists) return res.send({ success: false, message: "Already added to favorite" });
//       favourite.addedTime = new Date();
//       const result = await favouriteCollection.insertOne(favourite);
//       res.send({ success: true, insertedId: result.insertedId });
//     });
...
//     app.get("/orders/by-user/:email", verifyFBToken, async (req, res) => {
//       const email = req.params.email;
//       const result = await ordersCollection.find({ userEmail: email }).sort({ orderTime: -1 }).toArray();
//       res.send(result);
//     });
...
//       } catch (err) {
//         console.error(err);
//         res.status(500).send({ success: false, error: "Payment update failed" });
//       }
//     });
...
//       message.createdAt = new Date();
//       const result = await contactCollection.insertOne(message);
//       if (result.insertedId) return res.send({ success: true });
//       res.send({ success: false });
//     });
